...
def get_figures(url, soup):
	...
	for figure in figures:
		try:
			name = figure.find(class_=re.compile('figureLabel*')).text.encode('utf-8','ignore')
		except:
			name = ''
			pass
	...
	for table in tables:
		try:
			name = table.find('strong').text.encode('utf-8','ignore').strip()
		...
		caption = table.text.encode('utf-8','ignore').replace('| ','').strip()
		caption = re.compile('([\t\n]+)|(\s\s)+').sub(' ', caption)
		caption = re.compile('(\s\s)+').sub(' ', caption)
